...
StmtList* Parser::parseParamList() {
  ...
  if (parseTerminal(Token::comma, ",", false)) {
    ...
    if (next == nullptr) {
      ErrorReporter{source}.report(token(), "Error: param-list: extreneous comma");
...
Expr* Parser::parseUnaryExpr() {
  ...
  if (!expr) {
    ErrorReporter{source}.report(token(), "Error: unary-expr: expected expression");
    return nullptr;
  }
  if (!op) return expr;
...
Expr* Parser::parseInfixNone(int p) {
  ...
  if (!right) {
    ErrorReporter{source}.report(token(), "error: infix-none: expected expression");
    return nullptr;
  }
...
Expr* Parser::parseInfixLeft(int precedence) {
  ...
  continueParse = [this, &continueParse](int precedence, Expr* left) -> Expr* {
    ...
    if (!right) {
      ErrorReporter{source}.report(token(), "error: infix-expr: expected expression");
      return nullptr;
    }
...
ExprList* Parser::parseExprList() {
  ...
  if (!expr) {
    ErrorReporter{source}.report(token(), "Error: expr-list: expected expression");
    return nullptr;
  }
  ...
  if (parseTerminal(Token::comma, ",", false)) {
    ...
    if (!next) {
      ErrorReporter{source}.report(token(), "Error: expr_list: extreneous comma");
